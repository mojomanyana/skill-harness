...
import { codexCharterHash, validateCodexCharter, validateCodexSingleRequestCharter, assertCodexSingleRequestPlan, CODEX_RUNTIME_FILES } from '../../packages/adapters/dist/codex-subscription.js';
import { prepareProducerProduct } from '../../packages/adapters/dist/producer-product.js';
...
export function loadLaunch(path,mode,approvalPath){
 ...
 const product=config.version==='supervised-producer-product-launch-v1',single=config.version==='supervised-producer-single-request-launch-v1';
 closed(config,['version','profile','manifest','pins','limits',...(product?['product']:[])]);if(!['supervised-producer-launch-v1','supervised-producer-product-launch-v1','supervised-producer-single-request-launch-v1'].includes(config.version)||!['fixture','subscription-live'].includes(config.profile))throw Error('launch profile');
 const productPlan=product?prepareProducerProduct(config.product):null;
 ...
 else if(mode==='--fixture'){if(approvalPath||config.profile!=='fixture')throw Error('fixture cannot activate live');approval={scope:single?'fixture-single-request':'fixture',...(single?{approvedCalls:1}:{}),charterSha256:codexCharterHash(c),approvalId:'local-inert-only',expiresAt:Date.now()+c.limits.wallMs+60000,journalPath:m.journalPath};validate(c,approval);}
 else if(mode==='--live'){if(config.profile!=='subscription-live'||!approvalPath)throw Error('separate live approval required');const a=JSON.parse(readFileSync(approvalPath));closed(a,['version','launchSha256','approvedLiveCalls','executionApproval']);if(a.version!==(single?'supervised-producer-single-request-approval-v1':'supervised-producer-approval-v1')||a.launchSha256!==launchSha256||a.approvedLiveCalls!==approvedCalls||a.executionApproval.scope!==(single?'subscription-single-request':'subscription-live'))throw Error('exact launch approval mismatch');approval=a.executionApproval;validate(c,approval);if(approval.journalPath!==m.journalPath)throw Error('approved owner path');}
