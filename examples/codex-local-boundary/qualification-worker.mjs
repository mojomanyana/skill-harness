...
async function main(){
 ...
 const bindings=Object.fromEntries(c.invocations.map(i=>[i.model,{model:catalogue[i.model],stream}]));
 ...
 if(mode==='--fixture'){const f=await import('./qualification-fixture-ports.mjs');const p=f.fixturePorts(c,bindings);ports=p.ports;fixture=p.counters;}
 ...
 const budget=await createExperimentBudget({directory:'/out/budget',authorityDigest:launchSha256,limits:{maxAttempts:5,maxConcurrent:5,maxInputBytes:8192}}),owner=openResourceBudget(budget),completions=[];
 const source={owner,signal:controller.signal,producer:{...producer,startProducerIpc:async input=>{const run=await producer.startProducerIpc(input);completions.push(run.completion);return run;}},bindings:c.invocations.map(i=>({version:'producer-ipc-v1',budgetDigest:resourceBindingDigest(budget),orderId:'qualification-'+launchSha256.slice(0,16),experimentId:'qualification-'+launchSha256.slice(16,32),executionId:newExecutionId(),charterSha256:codexCharterHash(c),invocationId:i.id}))};
 const result=await executeProducerCodexQualification(config.manifest.journalPath,c,approval,ports,source);
...
main().catch(()=>{console.error('qualification worker refused or failed; original accounting retained, no retry');process.exitCode=1;});
