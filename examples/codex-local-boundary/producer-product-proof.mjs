...
product.protocol.draft.resourceMetric='wall_ms';
...
if(['cancel-source','fail-settlement'].includes(scenario)){
 assert.notEqual(child.status,0);assert.equal(existsSync('/out/result.json'),false);assert.ok(journal.some(e=>e.value.type==='producer-plan-failed'));assert.ok(!journal.some(e=>e.value.type==='producer-plan-settled'));assert.ok(!journal.some(e=>e.value.type==='subject-exchange-cost'||e.value.type==='retained'));
 ...
 const costs=journal.filter(e=>e.value.type==='subject-exchange-cost').map(e=>e.value);assert.equal(costs.length,6);assert.ok(costs.every(c=>Number.isFinite(c.wallMs)&&c.wallMs>=0&&c.metric==='wall_ms'&&c.excludes==='judges'));const retained=journal.filter(e=>e.value.type==='retained').map(e=>e.value);assert.equal(retained.length,6);assert.deepEqual(retained.map(c=>c.cost),costs.map(c=>c.wallMs));assert.ok(result.result.assessment.assessment.arms.every(a=>a.costUnit==='wall_ms'&&Number.isFinite(a.cost)));if(scenario==='objective-failure')assert.equal(result.result.assessment.assessment.cheapestEligible,null);
