...
function events(model='gpt-5.6-sol',text='4') {const item={id:'msg_fixture',type:'message',role:'assistant',status:'completed',content:[{type:'output_text',text,annotations:[]}]};return [
 ...
 {type:'response.output_text.done',output_index:0,content_index:0,item_id:item.id,text},
 {type:'response.content_part.done',output_index:0,content_index:0,item_id:item.id,part:item.content[0]},
...
let totalFetches=0;
for(const scenario of ['success','malformed','truncated','reordered','correlation','settings','wire-tamper','quota','retry','oversize','data-only','progress','unsupported-reasoning']) {
 ...
 let fetches=0;
 const fixture=async()=>{fetches++;totalFetches++;let es=events();if(scenario==='reordered')[es[1],es[2]]=[es[2],es[1]];if(scenario==='correlation')es[2].item_id='wrong';if(scenario==='settings')es.at(-1).response.model='gpt-5.5';if(scenario==='progress'){es.splice(1,0,{type:'response.in_progress',response:{id:'resp_gpt-5.6-sol',status:'in_progress'}});es=es.map((e,i)=>({...e,sequence_number:i}));}if(scenario==='unsupported-reasoning')es[1].item.type='reasoning';let body=scenario==='data-only'?': heartbeat\r\n\r\n'+es.map(e=>`data: ${JSON.stringify(e)}\r\n\r\n`).join(''):encode(es);if(scenario==='malformed')body='event: broken\ndata: {\n\n';if(scenario==='truncated')body=encode(es.slice(0,-1));if(scenario==='oversize')body='x'.repeat(16385);return new Response(scenario==='quota'?JSON.stringify({error:{message:'fixture quota exhausted'}}):body,{status:scenario==='quota'?429:200,headers:{'content-type':scenario==='quota'?'application/json':'text/event-stream'}});};
...
console.log(JSON.stringify({passed:15,fixtureFetches:totalFetches+5,liveCalls:0,authReads:0,scope:'actual installed serializer/compression/response decoder + durable host; text-only inert compatibility, not backend identity or live qualification'}));
