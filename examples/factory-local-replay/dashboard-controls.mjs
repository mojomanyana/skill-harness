...
export async function exerciseDashboardControls({root,host,config,budget,authority,ordinary,directory,domain,work}){
 ...
 const cancel={version:'ordinary-cancel-v1',requestId:`${domain}:ordinary-stop`,bindingDigest:ordinary.port.bindingDigest,expectedRevision:before.revision,target:before.children[0].target};authority.ordinary={bindingDigest:ordinary.port.bindingDigest,requestDigests:[ordinaryCancellationDigest(cancel)]};
 ...
 authority.dispatch.requestDigests.push(intentRequestDigest(request));const applied=await invoke('intent',request,`${domain}:host-revise`);if(applied.result.result.records[0].application!=='applied')throw Error('actual successor application failed');
 ...
 return {originalCancellation:'settled-original-caller',caller,ordinary:ordinary.port.inspect(),successor:{selection,scopeUnchanged:JSON.stringify(next.payload.snapshot.scope)===JSON.stringify(snapshot.scope),application:'applied',priorPrefixPreserved:true,acceptance:'not-assessed'},liveQualified:false};
}
