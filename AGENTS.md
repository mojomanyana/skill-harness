...
- **`contracts/pi-daddy/ledger/v2/`** — the pinned producer contract the `pi-daddy-v1` event adapter validates against, vendored byte-exact with per-artifact SHA-256. Its `README.md` says what is contract and what is harness-only, and how to re-pin. `node scripts/check-pi-daddy-contract.mjs` runs the four-fixture conformance check (free, offline). Never hand-edit `packages/adapters/src/pi-daddy-ledger-v2.ts` — it is generated.
