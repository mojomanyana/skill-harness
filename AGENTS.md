...
learning [status|import|review|trust|decide|adoption|outcome|guide]  guided retained learning — free, offline; explicit local writes, no model calls
...
screen <run-dir>...                       retained delivery-aware scenario/criterion rates — free, offline
...
- **`docs/factory/PRODUCT-GUIDE.md`** / **`docs/factory/STATUS.md`** — ordinary retained learning flow and current requirement register. Quality, independent trust, adoption authority, original registry activation and outcomes are separate. Historical excerpt feedback must not be promoted to full-artifact acceptance.
