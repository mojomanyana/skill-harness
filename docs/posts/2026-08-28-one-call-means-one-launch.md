...
automatic retry. An interrupted claim needs explicit continuation authority, and an
existing launch-attempt record is never replayed.
...
The local spool is canonical JSON, atomic writes, durable partial output, process-group
timeouts, and hash-chained lifecycle/accounting records. Those hashes detect torn or
edited evidence. They do not authenticate a signer or defeat the person who owns the
