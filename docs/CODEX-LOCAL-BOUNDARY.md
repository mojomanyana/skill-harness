...
The deliberately narrow supported stream is created → one assistant text item → text deltas → completed item → completed response, with optional progress/content-part/text-completion events and standard data-only, CRLF and comment framing. The measured optional reasoning prefix is also supported: exactly one leading reasoning item(index0), zero or one fully correlated completed summary part(index0), then exactly one final_answer assistant message(index1) with full text/content completion. Opaque encrypted strings may change between add/done, are capped8192bytes and never decoded; summary is capped4096bytes within the existing16384byte response limit. SDK thinking text/signature must match the privately validated final reasoning item; ONLY final text is emitted/scored. Evidence records the profile and verification booleans, never separate private reasoning. Unknown events, tools, reasoning-only, multiple/interleaved items and other reasoning shapes remain **unsupported**, never a qualified empty success. A missing/empty Content-Type on HTTP200 is compatible only after that strict, model-bound SSE validation succeeds. The decoder alone then receives derived `text/event-stream`; evidence retains `originalMediaType: missing` and `mediaTypeDerived: true`, never an upstream header claim. Explicit conflicting nonempty types still fail. This is not a general Codex stream implementation. HTTP/SSE bytes are exercised through a host-owned **inert fetch seam**, not a live socket: only an invalid fixture credential exists; auth loading, real network, redirects/fallback and SDK retries are unavailable. Ordinary Pi extension-bearing measurement rejection is unchanged.
