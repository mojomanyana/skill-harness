...
import { parseSpec, regradeRun, runSkillModel, type HarnessAdapter } from "../src/index.js";
...
it("skill_delivered surfaces absent bytes and costs zero judge calls (breaks if run trusts declared mode or judges through the gate)", async()=>{
 ...
 const adapter:HarnessAdapter={name:"pi",observesPrompts:true,available:async()=>true,run:async req=>{req.onPromptObservation?.({capture_version:"prompt-provenance-v1",request_index:0,raw_sha256:h,normalized_sha256:h,normalization_rule:"cwd-line-v1",bytes:1,contract_sha256:h,contract_bytes:1,contract_occurrences:0,mechanism:"pi-skill",status:"NOT-MEASURED"});return ">>> USER:\nhi\n\n<<< ASSISTANT:\nanswer\n"},judge};
 try{const {results,runDir}=await runSkillModel({spec,skillDir,specPath,adapter,model:{provider:"fake",model:"m"},modelToken:"fake:m",judge:{provider:"fake-judge",model:"j"},mode:"green",timestamp:"2026-09-04T00:00:00.000Z"});expect(results.schema).toBe(3);expect(judge).not.toHaveBeenCalled();expect(results.scenarios.every(s=>s.objective?.assertions.some(a=>a.kind==="skill_delivered"&&a.status==="NOT-MEASURED"))).toBe(true);expect(results.scenarios.every(s=>s.judge_verdict==="NOT-MEASURED")).toBe(true);expect(results.effective_grade).toMatchObject({passed:0,total:0,ship:false});expect(results.effective_grade.note).toMatch(/not measured/i);const regraded=await regradeRun({runDir,spec,adapter,judge:{provider:"fake-judge",model:"j"},specDir:join(skillDir,"tests")});expect(judge).not.toHaveBeenCalled();expect(regraded.scenarios.every(s=>s.judge_verdict==="NOT-MEASURED")).toBe(true);}finally{rmSync(skillDir,{recursive:true,force:true})}
});
