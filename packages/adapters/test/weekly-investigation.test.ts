...
it('prepares only bounded archive model inputs, preserves one original weekly selection and refuses tool/path expansion',()=>{const f=fixture(),job=createWeeklyInvestigation(f.directory,f.input);const request={kind:'producer-archive-model-v1' as const,manifestIds:[f.input.reader.manifestIds[0]],maxInputBytes:3000,proposalLimits:{subjectCalls:0,judgeCalls:0,wallMs:1000}};expect(()=>job.prepareModel({...request,manifestIds:['f'.repeat(64)]})).toThrow(/archive/);expect(()=>job.prepareModel({...request,maxInputBytes:1})).toThrow(/overbudget/);expect(job.inspect().state).toBe('prepared');const p=job.prepareModel(request);expect(job.prepareModel(request)).toEqual(p);expect(job.inspect().state).toBe('model-prepared');expect(()=>job.run({kind:'inert-v1',requests:[]})).toThrow(/claimed/);expect(()=>job.prepareModel({...request,maxInputBytes:2999})).toThrow(/changed/);});
