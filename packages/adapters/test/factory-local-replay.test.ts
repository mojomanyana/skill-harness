...
describe('two-domain local factory replay',()=>{
 ...
 it('connects real implemented APIs without presenting fixtures as live qualification',async()=>{
  ...
  for(const domain of result.domains){
   expect(domain.investigationSelectionId).toMatch(/^[a-f0-9]{64}$/);
   expect(domain.evaluationFrozen).toBe(true);
   expect(domain.calibration).toMatchObject({correct:1,resolved:1,reference:'synthetic-confirmation'});
