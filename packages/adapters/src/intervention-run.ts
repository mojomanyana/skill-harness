...
import { retainBlindIntervention } from './blind-intervention.js';
...
export function openInterventionRun(path:string){
 ...
 const api={configurations,
  ...
  casting(scope:CastingScope){if(learningHash(scope)!==learningHash(input.scope))throw Error('casting scope/version requires revalidation');const result=api.finish();return result.assessment.arms.map(arm=>({scope:learningCopy(scope),armId:arm.armId,configuration:configurations().find(c=>c.armId===arm.armId)!,conditionalState:arm.state,eligibleInert:arm.eligible,retainedCells:cells.filter((_,i)=>find('retained',arm.armId,i)).length,requiredCells:cells.length,panels:cells.map((_,i)=>find('panel',arm.armId,i)??null),cost:arm.cost,costUnit:arm.costUnit,latency:null,measuredAcceptance:null,escapes:null,measuredSamples:0,uncertainty:'unqualified inert/retained protocol; panel agreement is not accuracy',revalidateOn:['scope','model/effort/skill/prompt/configuration','rubric/judge policy','independent labels','delivery/effect transport'],routingDefault:null,liveQualified:false}));}
 };return api;
}
