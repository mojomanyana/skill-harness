...
export function openTrustLifecycle(directory:string){
 ...
 const previewExposure=(now:number)=>{const view=inspect(now),policy=input.exposure?{...input.exposure,attentionRemaining:Math.max(0,input.exposure.attentionRemaining-view.attentionUsed)}:null;
  const report=view.calibration.reports.find(r=>policy&&r.kind===policy.kind&&r.split===policy.split);
  return view.retired?{mode:'retire' as const,reason:'previously-retired'}:report?recommendExposure(report,policy,now):{mode:'silent' as const,reason:policy?'insufficient-independent-evidence':'policy-unavailable'};
 };
 return {inspect,configuration:()=>learningCopy(input),previewExposure,history:()=>journal.read(),previewCaseOutcome,
  linkCaseOutcome(predictionId:string,reference:LearningCaseReference,authorizedDigests:readonly string[],referenceAuthor?:string){if(referenceAuthor!==undefined&&!text(referenceAuthor))throw Error('explicit reference author required');const preview=previewCaseOutcome(predictionId,reference);if(!authorizedDigests.includes(preview.digest))throw Error('independent exact reference authority required');const s=replay();if(s.tip!==preview.tip)throw Error('stale case outcome snapshot');const old=s.outcomes.get(preview.outcome.referenceId);if(old){if(trustOutcomeDigest(old)!==trustOutcomeDigest(preview.outcome)||learningHash(s.caseLinks.get(old.referenceId)??null)!==learningHash(reference))throw Error('reference identity conflict');return;}
   journal.append(s.tip,{type:'outcome',outcome:preview.outcome,caseReference:preview.caseReference,authorityDigest:preview.digest,...(referenceAuthor===undefined?{}:{referenceAuthor})});},
  ...
  outcome(value:TrustReferenceOutcome,authorizedReferenceDigests:readonly string[],referenceAuthor?:string){if(referenceAuthor!==undefined&&!text(referenceAuthor))throw Error('explicit reference author required');const r=learningCopy(value);if(Object.keys(r).sort().join()!=='evidenceManifestId,kind,referenceId,targetId,value'||!['prediction','unflagged'].includes(r.kind)||!text(r.targetId)||!text(r.referenceId)||typeof r.value!=='boolean')throw Error('invalid reference outcome');const digest=trustOutcomeDigest(r);if(!authorizedReferenceDigests.includes(digest))throw Error('independent exact reference authority required');evidence(input.archiveRoot,r.evidenceManifestId);
   ...
   const old=s.outcomes.get(r.referenceId);if(old){if(trustOutcomeDigest(old)!==digest)throw Error('reference identity conflict');return;}journal.append(s.tip,{type:'outcome',outcome:r,authorityDigest:digest,...(referenceAuthor===undefined?{}:{referenceAuthor})});},
