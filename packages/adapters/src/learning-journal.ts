...
export function registerLearningStore(root:string,kind:'weekly'|'trust'|'intervention'|'access'|'workspace',key:string,target:string,binding:string){
 if(!/^[a-f0-9]{64}$/.test(key)||!isAbsolute(target)||!['weekly','trust','intervention','access','workspace'].includes(kind))throw Error('invalid learning registration');directory(root);directory(dirname(target));
...
export function verifyLearningStore(root:string,kind:'weekly'|'trust'|'intervention'|'access'|'workspace',key:string,target:string,binding:string){
