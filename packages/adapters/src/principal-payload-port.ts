...
export function createPrincipalPayloadPort(root:string,archiveId:string){
 ...
 return Object.freeze({version:'principal-check-payload-port-v1' as const,
  ...
  async read(raw:PrincipalPayloadReceipt,options:{max_bytes:number}){
   ...
   const result=readArchiveSource(root,receipt.object_id,options.max_bytes);if(result.status!=='available')throw Error('archive payload unavailable');
