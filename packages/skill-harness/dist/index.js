...
import { basename as basename3, dirname as dirname18, join as join43 } from "node:path";
...
function runPiJson(opts) {
  return new Promise((resolve22, reject) => {
    ...
    child2.on("close", (code) => {
      ...
      resolve22({ ...parsed, code, stderr: stderr.slice(0, MAX_STDERR_CHARS), providerFailure });
    });
  });
}
...
async function serveReview(opts) {
  ...
  await new Promise((resolve22) => server.listen(opts.port ?? 0, "127.0.0.1", resolve22));
...
function index_default(pi) {
  const moduleDir = dirname18(fileURLToPath3(import.meta.url));
  const assetsDir = basename3(dirname18(moduleDir)) === "skill-harness" ? join43(moduleDir, "..", "assets") : join43(moduleDir, "..", "..", "..", "assets");
