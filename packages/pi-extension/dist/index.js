...
import { dirname as dirname11, join as join36, resolve as resolve16, relative as relative5 } from "node:path";
...
function runPiJson(opts) {
  return new Promise((resolve17, reject) => {
    ...
    child2.on("close", (code) => {
      ...
      resolve17({ ...parsed, code, stderr: stderr.slice(0, MAX_STDERR_CHARS), providerFailure });
    });
  });
}
...
import { join as join33, dirname as dirname9 } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { spawn as spawn5 } from "node:child_process";
var __dirname = dirname9(fileURLToPath2(import.meta.url));
function templatePath(assetsDir) {
  if (assetsDir)
    return join33(assetsDir, "report.template.html");
  const candidates = [
    join33(__dirname, "..", "..", "..", "assets", "report.template.html"),
...
function gradeScriptPath(assetsDir) {
  return join33(dirname9(templatePath(assetsDir)), "report.grade.js");
}
...
function findTranscript(runDir, id) {
  ...
  if (files.length === 0)
    return null;
  if (files.length === 1)
    return readFileSync25(join33(runDir, files[0]), "utf8");
  return files.map((f) => `===== ${f} =====
${readFileSync25(join33(runDir, f), "utf8")}`).join("\n\n");
}
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/rejudge") {
        ...
        const specPath = join33(opts.skillDir, "tests", "specification.yaml");
        const spec = loadSpec(specPath);
        ...
        try {
          ...
          ensureResultsGitignore(join33(opts.skillDir, "tests", "results"));
      ...
      if (req.method === "POST" && url.pathname === "/save") {
        ...
        const spec = loadSpec(join33(opts.skillDir, "tests", "specification.yaml"));
        writeResults(column.runDir, patched, scoreContextFor(patched, spec));
        ensureResultsGitignore(join33(opts.skillDir, "tests", "results"));
        if (body.override != null) {
          preserveTranscript(join33(opts.skillDir, "tests", "results"), column.runDir, body.scenarioId);
        }
...
import { dirname as dirname10, join as join34, resolve as resolve15 } from "node:path";
function resolveSkillDir(cwd, arg) {
  if (arg) {
    const dir2 = resolve15(cwd, arg);
    if (existsSync24(join34(dir2, "tests", "specification.yaml"))) return dir2;
    throw new Error(`no tests/specification.yaml found at ${dir2}`);
  }
  let dir = cwd;
  for (; ; ) {
    if (existsSync24(join34(dir, "tests", "specification.yaml"))) return dir;
...
async function runViaExtension(opts) {
  ...
  const failedTranscripts = verdicts.filter((v) => v.verdict !== "PASS").flatMap((v) => findTranscriptFiles(summary.runDir, v.id, summary.results.mode).map((f) => join34(summary.runDir, f)));
  return {
...
async function runCapture(skillDir, ctx) {
  ...
  const specPath = join35(skillDir, "tests", "specification.yaml");
  ...
  const capturesDir = join35(skillDir, "tests", "captures");
...
${previewYaml}---`);
  ...
  writeFileSync11(join35(capturesDir, `${capture.id}.yaml`), yaml.dump(promoted, { lineWidth: -1, noRefs: true }), "utf8");
...
function writeCapture(capturesDir, capture, selected2, homeDir) {
  mkdirSync9(join35(capturesDir, ".local"), { recursive: true });
  const gitignore = join35(capturesDir, ".gitignore");
  ...
  const casePath = join35(capturesDir, `${capture.id}.yaml`);
  writeFileSync11(casePath, yaml.dump(capture, { lineWidth: -1, noRefs: true }), "utf8");
  const evidencePath = join35(capturesDir, ".local", `${capture.id}.evidence.json`);
  writeFileSync11(
    evidencePath,
    JSON.stringify(
      {
...
${card.failedTranscripts.join("\n")}`);
  ...
  if (sub === "judge") {
    const runDir = resolve16(ctx.cwd, positional[0] ?? ".");
    const testsDir = dirname11(dirname11(dirname11(runDir)));
    const spec = loadSpec(join36(testsDir, "specification.yaml"));
    const prev = existsSync26(join36(runDir, "results.yaml")) ? readResults(runDir) : null;
  ...
  if (sub === "coverage") {
    ...
    const report = computeCoverage({
      specDir,
      scenarios: spec.scenarios,
      baseFiles: [relative5(specDir, join36(skillDir, "SKILL.md")).split("\\").join("/")]
    });
  ...
  if (sub === "review") {
    const skillDir = resolveSkillDir(ctx.cwd, positional[0]);
    const spec = loadSpec(join36(skillDir, "tests", "specification.yaml"));
...
function index_default(pi) {
  const assetsDir = join37(dirname12(fileURLToPath3(import.meta.url)), "..", "..", "..", "assets");
