...
}else setTimeout(finish,Number(text)||0);
`;

function fixture(delay = 0, controlled = false) {
...
describe("qualification CLI lifecycle",()=>{
  ...
  it("the start caller exits while the detached supervisor continues",async()=>{
    ...
    try {
      ...
      // Release only this inert fixture, including on assertion failure. No PID guesses.
      writeFileSync(f.release,"release\n");
    }
    const done=cli(f,["qualification","poll","--spool",f.spool,"--id","invocation-cli-1","--wait-ms","5000","--interval-ms","10"],15000);
    expect(done.error).toBeUndefined();
    expect(done.status,done.stderr).toBe(0);
    expect(JSON.parse(done.stdout)).toMatchObject({phase:"terminal",terminal_status:"completed",attempt:1});
    expect(JSON.parse(cli(f,["qualification","validate","--spool",f.spool]).stdout)).toMatchObject({ok:true,terminal:1});
    expect(lines(f.count)).toBe(1);
  },30000);
